$(document).ready(
		function() {
			// 对表单绑定校验
			$('#form_order_add_edit').validationEngine('attach', {
						...
						$.ajax({
							...
							success : function(order) {
								...
								if (order) {
									...
									// 清空所有的校验信息
									$('.formError').remove();
									$('#indentCode').val(order.indentCode);
									$('#productNum').val(order.productNum);
									$('#bayCount').val(order.bayCount);
									$('#bayCount').val(order.bayCount);
									$('#totalPrices').val(order.totalPrices);
									$('#rowId').val(order.rowId);
									$('#action_info').html('修改');
								}
							}
						});
...
function saveOrUpdate() {
	...
	if (rowId) {// 如果主键有信息，则判断是修改
		$.ajax({
			type : 'post',
			url : 'order/doupdate',
			data : $('#form_order_add_edit').serialize(),
			dataType : 'json',
			success : function(result) {
				if (result) {
