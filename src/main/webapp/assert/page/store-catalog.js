...
//	对表单绑定校验
	$('#form_catalog_add_edit').validationEngine('attach',{
...
function saveOrUpdate() {
	...
	if(rowId){//如果主键有信息，则判断是修改
		$.ajax({
			type:'post',
			url:'catalog/doupdate',
			data:$('#form_catalog_add_edit').serialize(),
			dataType:'json',
			success:function(result){
				if(result){
					$('#modal_catalog').modal('hide');
					initTalbeData(1);
				}
			}
		});
