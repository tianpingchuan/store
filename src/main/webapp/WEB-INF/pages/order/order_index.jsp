							...
							<div class="col-auto my-1">
								<select class="form-control" name="indentCode">
									...
									<c:if test="${!empty indentOrderList}">
										<c:forEach items="${indentOrderList}" var="indent">
											<option value="${indent.indentCode}">${indent.indentCode}</option>
										</c:forEach>
									</c:if>
								</select>
							</div>
							<div class="col-auto my-1">
								<select class="form-control" name="productNum">
									...
									<c:if test="${!empty productOrderList}">
										<c:forEach items="${productOrderList}" var="product">
...
<div class="modal fade" id="modal_order" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<form id="form_order_add_edit">
				<div class="modal-body">
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">订单编号</label>
						<div class="col-sm-7">
							<select class="form-control" name="indentCode" id="indentCode">
								<c:if test="${!empty indentOrderList}">
									<c:forEach items="${indentOrderList}" var="indent">
										<option value="${indent.indentCode}">${indent.indentCode}</option>
									</c:forEach>
